...
HDLC_BUF hdlc_buf;
HDLC_BUF * hdlc = &hdlc_buf;

HDLC_DLMS_exchange * hdlc_exch = 0;
void HDLC_BUF_tx(uint8_t c)
{
    hdlc_exch->tx_buf.append(c);
}
...
//                log_1 << out_pdu_data_reader.first << qPrintable(QJsonDocument::fromVariant(out_pdu_data_reader.second).toJson());
        ...
        case DLMS_GET_COMMAND_TYPE_NEXT_DATA_BLOCK: {
            ...
            block_index <<= 8;
            block_index += *pb_data++;
//            log_5 << "last_block" << last_block;
//            log_5 << "block_index" << block_index;
...
void HDLC_print_header(HDLC_BUF *hdlc)
{
    QString str("H(%1 %2) Len(%3) SaddrF(%4) Caddr(%5) Ctrl(RRR=%6 PF=%7 SSS=%8 I=%9 = %10) Len(%11) HCS(%12 %13) FCS(%14 %15)");
    QString cntrl;
    ...
    log_6 << str.arg(hdlc->format_type, 0, 16).arg(hdlc->segmentation).arg(hdlc->frame_length).arg(saddr).arg(caddr)
...
//        log_1 << "clnt" << i;
    ...
    {
        log_6 << "TX" << hdlc_exch->tx_buf.left(250).toHex().toUpper();
//        HDLC_print_header(hdlc);
        emit hdlc_exch->signal_HDLC_to_device(hdlc_exch->tx_buf);
        hdlc_exch->tx_buf.clear();
    }

    HDLC_Clear(hdlc);
}
void HDLC_Send_LLC(HDLC_BUF *hdlc)
{
    hdlc->segmentation = 0;
    hdlc->control.bits.is_n_I = 0;
    hdlc->control.bits.P_F = 1;
    hdlc->LLC_flag = 1;
    hdlc->control.bits.RRR = hdlc->control_clt.bits.RRR;
    hdlc->control.bits.SSS = hdlc->control_clt.bits.SSS;
    HDLC_Send(hdlc);
    hdlc->control.bits.RRR = ++hdlc->control_clt.bits.RRR;
    hdlc->control.bits.SSS = hdlc->control_clt.bits.SSS++;
}
void HDLC_MAC_stringify_controller_cmd(HDLC_BUF *hdlc, control_field_enum cf)
{
    switch (cf) {
    case cf_SSS_RR: {
        hdlc->segmentation = 1;
...
{
    if (!pdu.isEmpty()) {
        hdlc->data_len = pdu.size();
        hdlc->data_index = 0;
        memmove(hdlc->data_buf, pdu.data(), hdlc->data_len);
    }
    HDLC_Send_LLC(hdlc);
//    log_1 << tx_buf.left(250).toHex().toUpper();
//    emit signal_HDLC_to_device(tx_buf);
}
void HDLC_MAC_LLC_parser(HDLC_BUF *hdlc, uint8_t c)
{
...
//    }
    if (c == 0x7E && hdlc->mode_frame > 2 && hdlc->frame_length - hdlc->frame_byte_counter < 1) { // end packet
        ...
        if ((hdlc->HCS == hdlc->calc_HCS) && (hdlc->crc == hdlc->FCS) && (hdlc->control.control || hdlc->data_len)) {
            HDLC_print_header(hdlc);
            hdlc_exch->HDLC_MAC_parser_controller(hdlc);
//            DLMS_reader(hdlc, DLMS_reader_CAUSE_rx, 0);
//            hdlc_exch->HDLC_MAC_parser_controller(hdlc);
        }
...
{
     log_4 << "TX PDU" << pdu.toHex().toUpper();
//    HDLC_Clear(hbuf_in);
...
//    hdlc_in_buf_data += QByteArray((const char*)hdlc->data_buf, hdlc->data_len);
    QByteArray hdlc_in_buf_data = QByteArray((const char*)hdlc->data_buf, hdlc->data_len);
    if (hdlc->segmentation) {
        log_5 << "segmentation";
        hdlc->control.bits.RRR = hdlc->control_clt.bits.RRR++;
        hdlc->control.bits.SSS = cf_SSS_RR;
...
void HDLC_set_addr(HDLC_BUF *hdlc, uint32_t server, uint32_t client)
{
    HDLC_Clear(hdlc);

    hdlc->clnt.bytes4.addr = client;
    hdlc->clnt.bytes4.flag = 0;
    if (hdlc->clnt.bytes4.addr) hdlc->clnt.bytes4.flag = 1;
    if (hdlc->clnt.bytes4.addr > 0x7F) hdlc->clnt.bytes4.flag = 2;
    if (hdlc->clnt.bytes4.addr > 0x3FFF) hdlc->clnt.bytes4.flag = 4;
    hdlc->serv.bytes4.addr = server;
    hdlc->serv.bytes4.flag = 0;
    if (hdlc->serv.bytes4.addr) hdlc->serv.bytes4.flag = 1;
    if (hdlc->serv.bytes4.addr > 0x7F) hdlc->serv.bytes4.flag = 2;
    if (hdlc->serv.bytes4.addr > 0x3FFF) hdlc->serv.bytes4.flag = 4;
...
//    hbuf_out->timeout_connect = HDLC_TIMEOUT_CONNECT;
    ...
    log_1 << server << client;
    this->pass = pass;
    HDLC_set_addr(hdlc, server, client);
    HDLC_MAC_stringify_controller_cmd(hdlc, cf_SNRM);
...
void HDLC_DLMS_exchange::slot_vPDU_to_device(QVariant vpdu)
{
    QVariantMap vm = vpdu.toMap();
    if (vm.contains("aarq") && hdlc->clnt.bytes4.addr > 0x10) {
...
void HDLC_DLMS_exchange::slot_PDU_to_device(QByteArray pdu)
{
    log_4 << "TX PDU"<< pdu.toHex().toUpper();
    HDLC_MAC_stringify_controller_PDU(hdlc, pdu);
}
